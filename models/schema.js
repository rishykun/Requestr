...
ReviewSchema.statics.addReview = function (writerUsername, victimUsername, text, rating, requestId, userModel, requestModel, cb) {
  var that = this;

  userModel.getUser(writerUsername, function (err, writerObj) {
    if (err) {
      cb(err);
    } else {
      userModel.getUser(victimUsername, function (err, writerObj) {
        if (err) {
          cb(err);
        } else {
          requestModel.getRequestById(requestId, function (err, requestObj) {
            if (err) {
              cb(err);
            } else {
              that.create({
...
RewviewSchema.statics.getReviewsByVictimId = function (victimUsername, userModel, cb) {
  var that = this;

  userModel.getUser(victimUsername, function (err, victimObj) {
    if (err) {
      cb(err);
    } else {
      that.find({"victim": victimObj._id}).populate("writer").populate("victim").exec(function (err, reviewQuery) {
        if (err) {
          cb(err);
        } else {
          cb(null, reviewQuery);
...
ReviewSchema.statics.reviewExistsForRequestAndWriter = function (requestId, writerUsername, requestModel, userModel, cb) {
  var that = this;
